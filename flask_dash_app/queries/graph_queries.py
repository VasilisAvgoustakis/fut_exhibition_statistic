"""
SQL queries for graph data in the Flask Dash application.
"""

# Query for total scans per token station
QUERY_TOTAL_SCANS_TK = """
SELECT 
...
# Query for average scans per month
AVG_SCANS_PRO_MONAT = """
SELECT 
    station, 
    name, 
...
# Query for total scans per region
TOTAL_SCANS_PER_REGION = """
SELECT area, COUNT(station) AS station_number_per_area, 
...
# Query for average scans per region per station
...
# Query for average time per visitor
AVG_TIME_PER_VISITOR = """
SELECT ROUND((SUM(technology)/ COUNT(DISTINCT date))* 60,0) AS Technik, 
...
# Query for total visitors per day
...
# Query for average scans per visitor
...
# Query for scans per visitor percentile
SCANS_PER_VISITOR_PERCENTILE = """
WITH scans_per_code AS (
                                SELECT
                                    scan_band_code,
                                    scan_date,
                                    COUNT(*) AS scans
...
# Query for vote scans per question
...
# Query for visitor paths
VISITOR_PATHS = """
SELECT scan_band_code, 
...
QUERIES = [
    ...
    VOTE_SCANS_PER_QUESTION,
    VISITOR_PATHS,
    VISITOR_PATHS  # Used twice for different visualizations
]